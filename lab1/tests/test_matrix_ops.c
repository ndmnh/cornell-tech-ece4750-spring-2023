...
void test_matmul_square_matrices(void)
{
    ...
    // compare_matrices(matmul_blocking(matrix_1, matrix_2, 2, 2, 2, 2), matrix_3, 2, 2);
    ...
    // compare_matrices(matmul_blocking(matrix_4, matrix_5, 5,5,5,5), matrix_6, 5, 5);
    ...
    // big size, 4 * 32
    int lg_input_size = 8192;
    float lg_matrix_1_val[lg_input_size];
    for(int i = 0; i < lg_input_size; i++) {
        lg_matrix_1_val[i] = rand();
    }
    float **lg_matrix_1 = matrix_generator_2d(lg_matrix_1_val, 64, 128);

    float lg_matrix_2_val[lg_input_size];
    for(int i = 0; i < lg_input_size; i++) {
        lg_matrix_2_val[i] = rand();
    }
    float **lg_matrix_2 = matrix_generator_2d(lg_matrix_2_val, 128, 64);

    matmul_blocking(lg_matrix_1, lg_matrix_2, 64, 128, 128, 64);
